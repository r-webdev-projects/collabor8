...
class UserTest < ActiveSupport::TestCase
  ...
  def setup
    ...
    @user = User.new({:email => "#{foo}@gmail.com", :first_name => "someFirstName", 
    ...
    # save(:validate) is needed only when you use email validation, I think?
      #no, actually you've got to have it. Gotta save, and gotta ( => false).
    # as a side-note, when .save() is used -  foo == true. Not exactly sure why
    puts @user[:email]
  end
  ...
  test "first_name has to be proper length" do
    ...
    names.each do |invalid_name|
      ...
      assert_not @user.valid? "#{invalid_name} shouldn't be valid" # shows which is wrong
    end
  end
end
